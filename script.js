...
fetch('https://kanjisagashi.jitakuapp.com/randomVisual.bin')
  ...
  .then(arrayBuffer => {
    visualScores = new Uint8Array(arrayBuffer);
    // Use the byte array here
    console.log(visualScores);
  })
  .catch(error => {
    console.error('Error:', error);
  });

fetch('https://kanjisagashi.jitakuapp.com/randomSemantic.bin')
  .then(response => response.arrayBuffer())
  .then(arrayBuffer => {
    semanticScores = new Uint8Array(arrayBuffer);
    // Use the byte array here
    console.log(semanticScores);
  })
  .catch(error => {
    console.error('Error:', error);
  });

fetch('https://kanjisagashi.jitakuapp.com/randomReading.bin')
  .then(response => response.arrayBuffer())
  .then(arrayBuffer => {
    readingScores = new Uint8Array(arrayBuffer);
    // Use the byte array here
...
function checkGuess() {
  var guess = document.getElementById("guessInput").value;
    ...
    if (guess === hiddenCharacter.character) {
      ...
      if (index != -1) {
        ...
        createListElement("list3", "score3");
...
function updateChart(newData) {
  chart.data.labels.push(newData.kanji);
  chart.data.datasets[0].data.push(newData.score1);
  chart.data.datasets[1].data.push(newData.score2);
  chart.data.datasets[2].data.push(newData.score3);
  chart.update();
}
...
function getKanjiNumber () {
  ...
  return simpleHash(hash) % kanjiArray.length
}
...
// Adding a new kanji with scores
function addKanji(kanji, score1, score2, score3) {
  var newData = { kanji: kanji, score1: score1, score2: score2, score3: score3 };
  previousGuess.push(newData);
  updateChart(newData);
}



function createListElement(listId, scoreType) {
    var listElement = document.getElementById(listId);
    listElement.innerHTML = ""; // Clear existing content

    var sortedList;

    // Sort the kanji list by the specified score
    sortedList = previousGuess.slice().sort(function(a, b) {
        return b[scoreType] - a[scoreType];
    });
